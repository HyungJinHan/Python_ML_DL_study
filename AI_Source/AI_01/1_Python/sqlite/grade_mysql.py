from grade_mysql_class import Grade
import pymysql
# 파이썬에서의 MySQL의 경우 SELECT를 실행할 때 반드시 fetchall을 해야 처리가 잘됨
...
def input_func():
    ...
    obj.setAvg()
    obj.setGrade()
    # sql = "INSERT INTO grade_mysql (id, name, kor, eng, math, sum, avg, grade) values (%s, %s, %s, %s, %s, %s, %s, %s)"
    ...
    for row in res:
        if id_select == row[0]:
            ...
            if avg >= 90:
                ...
                grade = '가'
            dbcursor.execute('UPDATE grade_mysql SET kor = %s, eng = %s, math = %s, sum = %s, avg = %s, grade = %s WHERE id = %s',
...
def delete_func():
    ...
    for row in res:
        if id_select == row[0]:
            dbcursor.execute('DELETE FROM grade_mysql WHERE id = %s', (id_select,))
            dbconn.commit()
            dbcursor.close()
            dbconn.close()
            print('\n해당 학번의 정보가 삭제되었습니다.')
